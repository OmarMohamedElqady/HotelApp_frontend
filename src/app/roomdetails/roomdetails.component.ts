...
export class RoomdetailsComponent {
  ...
  minDate: string = new Date().toISOString().split('T')[0]; // get the current date in this format "yy-mm-dd"
  ngOnInit(): void {
  ...
  fetchRoomDetails(roomId: string): void {
    this.apiService.getRoomById(roomId).subscribe({
      next: (res: any) => {
        console.log('Room Result : ' + res.room);
  ...
  acceptBooking(): void {
    ...
    // we are building our body subject
    const booking = {
    ...
    this.apiService.bookRoom(booking).subscribe({
      next: (res: any) => {
        if (res.status === 200) {
          this.message =
            'Your Booking is Successful. An Email of your booking detais and the payment link has been send to you';
          setTimeout(() => {
            this.message = null;
      ...
      error: (err) => {
        this.showError(
          err?.error?.message || err?.message || 'Unable to make a booking'
        );
      },
    });
  }
